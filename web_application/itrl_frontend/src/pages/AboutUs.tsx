...
const About = () => {
    ...
    const translateAlgorithm = async () => {
        try {
            const translationEndpoint = language === 'english' ? 'http://127.0.0.1:8080/api/translateError/' : 'http://127.0.0.1:8080/api/translateAbout/';
            const translatedAboutText = await translateText(aboutText, translationEndpoint);
            const translatedHeadingText = await translateText(headingText, translationEndpoint);
            const translatedTeamHeadingText = await translateText(teamHeadingText, translationEndpoint);
            const translatedButtonText = language === 'english' ? 'Translate to English' : 'ಕನ್ನಡಕ್ಕೆ ಅನುವಾದಿಸಿ';

            const translatedMembers = await Promise.all(teamMembers.map(async (member) => {
                const translatedName = await translateText(member.name, translationEndpoint);
                const translatedDescription = await translateText(member.description, translationEndpoint);
                return {
                    ...member,
                    name: translatedName,
